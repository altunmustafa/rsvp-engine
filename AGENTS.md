...
- Before opening a PR, run `pnpm verify` and review the complete branch diff against `main` with a dedicated reviewer. Validate the exact proposed PR title with `printf '%s\n' "$PR_TITLE" | pnpm exec commitlint`; do not open the PR until it passes.
